...
const CustomerProfile = ({ showNotification }) => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setEditedProfile(response.data); // Initialize editedProfile with fetched data
      } catch (err) {
  ...
  const handleSave = async () => {
    try {
      await axios.put(
        "http://localhost:5000/api/customers/profile", // Changed /api/auth/user to /api/customers/profile
        editedProfile,
        { headers: { Authorization: `Bearer ${token}` } }
      );
